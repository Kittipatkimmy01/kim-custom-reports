...
class WizardPurchaseVatReport(models.TransientModel):
    _name = 'wizard.purchase.vat.report'
    _description = 'Wizard Purchase Vat Report'
    ...
    def default_get(self, fields):
        res = super(WizardPurchaseVatReport, self).default_get(fields)
        company = self.env.company
        account_purchase_tax_id = company.account_purchase_vat_report_id
#         vat_purchase = False
...
#         if vat_purchase:
        if account_purchase_tax_id:
            res['account_id'] = account_purchase_tax_id.id
        return res
    ...
    def print_report(self):
        ...
        if xlwt:
            ...
            worksheet = workbook.add_sheet('Purchase Vat Report')
...
#             company = self.env['res.company'].browse(1)
            ...
            purchase_move = False
            header_name = u"รายงานภาษีซื้อ (Purchases Tax Report)"
            worksheet.write_merge(row - 4, row - 4, col, col + 9, header_name, h0_style)
            worksheet.write_merge(row - 4, row - 4, col + 10, col + 11, u"หน้า", h0_style)
            worksheet.write_merge(row - 3, row - 3, col, col + 1, u"ชื่อผู้ประกอบการ", base_number_style)
            worksheet.write_merge(row - 3, row - 3, col + 2, col + 9, company_name, h3_style)
            worksheet.write_merge(row - 3, row - 3, col + 10, col + 11, u"เลขประจำตัวผู้เสียภาษี", h0_style)
            worksheet.write_merge(row - 3, row - 3, col + 12, col + 12, company.vat, h0_style)
            worksheet.write_merge(row - 2, row - 2, col, col + 1, u"ชื่อสถานประกอบการ", base_number_style)
            worksheet.write_merge(row - 2, row - 2, col + 2, col + 9, company_name, h3_style)
            worksheet.write_merge(row - 2, row - 2, col + 10, col + 11, u"สาขา", h0_style)
            worksheet.write_merge(row - 2, row - 2, col + 12, col + 12, u"00000", h0_style)
            worksheet.write_merge(row - 1, row - 1, col, col + 1, u"ขื่อที่อยู่สถานประกอบการ", h2_style)
            worksheet.write_merge(row - 1, row - 1, col + 2, col + 9, company.street, h1_style)
            worksheet.write_merge(row - 1, row - 1, col + 10, col + 11, u"เดือน/ปี", h0_style)
            worksheet.write_merge(row - 1, row - 1, col + 12, col + 12, u"{0}/{1}".format(month_val,years), h0_style)
            ...
            row += 1
            # descending_range = taxs.sorted(key=lambda r: r.move_id.name,reverse=True)
            total_base = total_tax = total_amount = 0.0
            tax_list = []
#             taxs = AccMoveLine.search(domain,order=order)
#             for tax in taxs:
#                 partner_name=partner_number=partner_branch=move_name=False
#                 base = 0.0
#                 base =tax.tax_base_amount
#                 tax_amount = tax.amount_currency
#                 move_name = tax.move_id.name
#                 partner_name = tax.partner_id.name
#                 partner_number = tax.partner_id.vat or '-'
#                 partner_branch = tax.partner_id.branch
#                 tax_date = datetime.strftime(tax.date, "%d/%m/%Y")
#                 tax_list.append({
#                     'date': tax_date,
#                     'vat_ref': tax.vat_ref or "-",
#                     'origin_ref': move_name or "-",
...
#                 })
            ...
            for taxl in tax_lns:
                ...
                if obj._name == 'account.petty.payment':
                    move_name = obj.number
                else:
                    move_name = obj.name
...
#                     worksheet.write(row, col + 12, tax.invoice_id.name or '', base_style)
            ...
            workbook.save(fp)
            fp.seek(0)
            report_name = "Purchase_Vat_Report"
            ...
            return {
                ...
                'url': '/web/content/wizard.purchase.vat.report/%s/data/%s?download=true' %(self.id,filename),
            }
#             return {
#                 'type': 'ir.actions.act_window',
#                 'res_model': 'wizard.purchase.vat.report',
#                 'view_mode': 'form',
#                 'view_type': 'form',
#                 'name': 'Report',
#                 'res_id': self.id,
